...
from app.config import LLMSettings, get_config
...
class LLM:
    ...
    def __new__(
        ...
        usage: Optional[str] = "default",
    ):
        if config_name not in cls._instances:
            instance = super().__new__(cls)
            instance.__init__(config_name, llm_config, usage)
    ...
    def __init__(
        ...
        if not hasattr(self, "client"):  # Only initialize if not already initialized
            ...
            elif usage == "reasoning":
                llm_config = llm_config.get("reasoning", llm_config["reasoning"])
            elif usage == "retrieval":
                llm_config = llm_config.get("retrieval", llm_config["retrieval"])
            else:
                raise ValueError(f"Invalid usage type: {usage}")
            # llm_config = llm_config.get(config_name, llm_config["default"])
    ...
    async def ask(
        ...
        extra_body: Optional[dict] = {"enable_thinking": True, "thinking_budget": 3000},
    ) -> str:
        """
    ...
    async def ask_tool(
        ...
        **kwargs,
        ...
        try:
            ...
            if tools:
                for tool in tools:
                    tools_tokens += self.count_tokens(json.dumps(tool))
